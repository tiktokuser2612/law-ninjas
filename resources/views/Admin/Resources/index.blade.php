              ...
              <div class="col-md-6">
                <div class="breadcrumb-wrapper mb-30">
                  <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                      <a href="{{url('/admin/Resources/add')}}" class="main-btn primary-btn btn-hover">Add Resources</a>
                    </ol>
                  </nav>
                </div>
              </div>
                  ...
                  <table class="table">
                    ...
                    @foreach ($data as $row)
                        ...
                        <td>{{$row->id}}</td>
                          <td class="min-width">
                              {{-- @php
                              $resource = \App\Models\ResourcesImage::where('resources_id',$row->id )->first();
                              @endphp --}}
                               <div class="thumbnail">
                                  <img src="{{url('storage/app/public/Admin/Resources/'.$row->image)}}" style="width:100%">
                               </div>
                          </td>
                        <td class="min-width">
                            <p>{{$row->title}}</p>
                        ...
                        <td class="min-width">
                           @php $prodID= Crypt::encrypt($row->id); @endphp
                            <a href="{{url('/admin/Resources/delete/'.$prodID)}}" class="status-btn close-btn delete-confirm"><i class="lni lni-trash-can"></i></a>
                            <a href="{{url('/admin/Resources/edit/'.$prodID)}}" class="status-btn success-btn"><i class="lni lni-pencil"></i></a>
                        </td>
                        <td>
                          <label class="switch">
      ...
      then(function(value) {
          if (value) {
              $.ajax({ 
                type: "POST", 
                dataType: "json", 
                url: "{{url('admin/updateResourcesStatus')}}", 
