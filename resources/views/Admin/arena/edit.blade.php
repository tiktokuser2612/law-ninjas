        ...
        <div class="container-fluid">
          ...
          <div class="row justify-content-center d-flex">
            <div class="col-xl-8 col-md-12 ">
              ...
              <form id="fileUploadForm" action="{{url('/admin/arena/update/'.$prodID)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="card-style mb-30">
                  <div class="d-flex align-items-center mb-2">
                      <div class="avatar-upload">
                        ...
                        <div class="avatar-preview">
                            <div id="imagePreview" style="background-image:url({{url('storage/app/public/Admin/Arena/'.$data->image)}})">
                            </div>
                        </div>
                         @if ($errors->has('image'))
                  ...
                  <div class="input-style-1">
                      <label>Name</label>
                      <input type="text" placeholder="Arena Name" name="name" value="{{$data->name}}">
                      @if ($errors->has('name'))
                          <span class="help-block font-red-mint">
                  ...
                  <div class="input-style-1">
                     ...
                     <select id="myselection" name="password_type"> 
                        @if ($data->password_type == "none")
                          <option  value="none" {{ $data->password_type == $data->password_type ? 'selected' : '' }}>{{ $data->password_type }}</option>
                          <option value="password">Password</option>
                        @else
                          <option  value="password" {{ $data->password_type == $data->password_type ? 'selected' : '' }}>{{ $data->password_type }}</option>
